...
if($upfile_size>5000000){
      ...
      if(!move_uploaded_file($upfile_tmp_name, $uploaded_file)){// 이동할 파일 시작, 도착
        ...
        $sql="insert into review set rv_title = '".addslashes(htmlspecialchars($_POST['rv_title']))."', rv_content = '".addslashes(htmlspecialchars($_POST['rv_content']))."', u_id = '".$_SESSION['u_id']."', rv_date = now(),rv_file_upload1='".$upfile_name."',rv_file_copy1='".$copied_file_name."'";
...
<script>
//alert('글이 저장 되었습니다.');
location.replace('review_view.php?no=<?=$insertno?>');
</script>
<?php
}
?>
